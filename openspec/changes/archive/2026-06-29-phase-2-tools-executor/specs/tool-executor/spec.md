...
The single ordered path every tool call flows through. It is the project's safety
spine: one chain, no second door. Phase 2 builds the full chain with its safety
stages present but inert, so later phases arm a stage without re-routing the path.
