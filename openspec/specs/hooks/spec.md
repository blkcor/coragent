...
A before-tool hook SHALL be able to allow, block, or replace the tool arguments
before permission, sandbox, or tool execution occurs.

#### Scenario: Before-tool block short-circuits downstream stages
- **WHEN** a before-tool hook blocks a tool call
- **THEN** permission is not consulted
- **THEN** the sandbox and tool body do not run
